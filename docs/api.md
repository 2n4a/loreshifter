...
Токен хранится в cookie `session` (помечен как `HttpOnly` и `Secure`) и/или может быть передан в заголовке `Authorization: Bearer <token>`.

Если ручка не найдена, возвращается ошибка 404.
Если метод не поддерживается, возвращается ошибка 405.

Все методы, требующие авторизации, проверяют наличие токена в cookie `session` или в заголовке `Authorization`, сверяют его с пользователем в базе данных и возвращают 401, если у пользователя нет доступа к ресурсу. В этом случае возвращается объект типа `ServiceError` с кодом `Unauthorized` и сообщением об ошибке.
...
Возвращает URL для редиректа на страницу авторизации от провайдера.
Параметр `provider` обязателен. Если `redirect=1`, сразу делает редирект, иначе возвращает JSON `{url: string}`.
...
### Параметры
- `id` - число, идентификатор пользователя. Если равен `0`, возвращается информация о себе (требует авторизации). Также можно использовать `GET /user/me`.
...
TODO: пока откладываем реализацию, потому что не хочется обновлять имя пользователя
в JWT.
...
- `order` - тип сортировки. У нас пока будет только `lastUpdatedAt`.
- `sort` - направление сортировки. `asc` - сначала старые, потом новые, `desc` - наоборот.
- `search` - поисковый запрос, пока не пишем.
...
### Ответ
В случае успеха возвращает объект типа `WorldOut[]` с кодом 200.
...
- `joined` - если `1`, вернуть только игры, в которых авторизованный пользователь сейчас присоединен (active).
...
Если игра не найдена, или у пользователя нет доступа к ресурсу, возвращает 404, код ошибки `GameNotFound`.

У пользователя есть доступ к публичным играм, к неопубликованным играм созданным им,
...
Если `maxPlayers` указан, но он меньше чем игроков в игре, возвращает 400, код ошибки `GameMaxPlayersTooSmall`.
...
Пользователь может быть присоединен к нескольким активным играм одновременно.
...
Если игрок уже в этой игре, возвращает объект типа `Game` с кодом 200 (идемпотентно).

## POST `/game/{id}/leave`

Авторизация: обязательна.

Выйти из игры с идентификатором `id`.

### Ответ

В случае успеха возвращает пустой объект с кодом 200.

Если игрок сейчас не в игре, возвращает пустой объект с кодом 200 (идемпотентно).
...
Все методы в этом протоколе работают с конкретной игрой, в которую играет пользователь. Соответственно, все методы требуют авторизации. Если пользователь не авторизован, возвращает 401, код ошибки `Unauthorized`. Если пользователь не в игре, возвращает 400, код ошибки `PlayerNotInGame`.

В пути каждого запроса должен быть указан идентификатор игры: `/game/{id}/...`.

## GET `/game/{id}/state`

Возвращает текущее состояние указанной игры. Возвращает объект типа `GameState`.
...
## GET `/game/{id}/chat/{chatId}`

Получить сегмент чата указанной игры. Возвращает объект типа `ChatSegment`.
...
## POST `/game/{id}/chat/{chatId}/send`

Отправить сообщение в чат указанной игры. В теле запроса содержится объект типа `MessageIn`.
...
Если персонаж не создан, возвращает 400, код ошибки `CharacterNotReady`.
...
Авторизация происходит либо через cookie `session`, либо через параметр запроса `lazy_auth=true`.
Если используется `lazy_auth=true`, то первым сообщением от клиента должно быть сообщение типа `auth` с токеном.

Все сообщения приходят в формате JSON и имеют формат `{type: string, payload: any}`.

### Сообщения от клиента

#### `{type: "auth", payload: string}`

Отправить токен авторизации. Используется только если при подключении был передан параметр `lazy_auth=true`.
...
#### `{type: "GameStatusEvent", payload: {game_id: number, new_status: string}}`

Уведомление об изменении статуса игры.

#### `{type: "PlayerJoinedEvent", payload: {game_id: number, player: Player}}`

Уведомление о присоединении игрока.

#### `{type: "PlayerLeftEvent", payload: {game_id: number, player: Player}}`

Уведомление о выходе игрока.

#### `{type: "PlayerKickedEvent", payload: {game_id: number, player: Player}}`

Уведомление о том, что игрок был выгнан.

#### `{type: "PlayerReadyEvent", payload: {game_id: number, player_id: number, ready: boolean}}`

Уведомление об изменении статуса готовности игрока.

#### `{type: "GameChatEvent", payload: {game_id: number, chat_id: number, owner_id: number | null, event: ChatEvent}}`

Уведомление о событии в чате. `event` содержит детали события чата.

Поле `event` может иметь следующую структуру (в зависимости от типа события чата, который не передается явно в поле `type`, но может быть определен по полям):

*   **Отправка сообщения**: `{message: {msg: Message, prev_id: number | null, next_id: number | null}}`
*   **Редактирование/Удаление сообщения**: `{message: Message}` (без `msg`, `prev_id`, `next_id`)
*   **Обновление подсказок**: `{suggestions: string[]}`
